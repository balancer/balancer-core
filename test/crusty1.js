...
deployTestScenario = async function(web3, accts, log) {
    ...
    env.acoin = await pkg.deploy(web3, admin, "BToken", [web3.utils.toHex("A")]);
    log(`${env.acoin._address} = deploy BToken`);
    env.bcoin = await pkg.deploy(web3, admin, "BToken", [web3.utils.toHex("B")]);
    log(`${env.bcoin._address} = deploy BToken`);
    env.ccoin = await pkg.deploy(web3, admin, "BToken", [web3.utils.toHex("C")]);
